...
class ServerManager {
    ...
    async fetchAdvanced(identifier) {
        const token = this._auth?.accessToken;
        if (!token) {
            ServerUtils_1.default.error(this._manager, `[${identifier}] Failed To Fetch Advanced Information: No Access Token`);
            return null;
        }
        const server = this._servers.get(identifier);
        if (!server) {
            ServerUtils_1.default.error(this._manager, `[${identifier}] Failed To Fetch Advanced Information: Invalid Server`);
            return null;
        }
        ...
        try {
            ...
            if (!response.ok) {
                ServerUtils_1.default.error(this._manager, `Failed To Fetch Advanced Information: HTTP ${response.status} ${response.statusText}`);
                return null;
            }
            const json = await response.json();
            const advanced = json?.data?.cfgContext?.ns;
            if (!advanced) {
                ServerUtils_1.default.error(this._manager, 'Failed To Fetch Advanced Information: No Data');
                return null;
            }
            ...
            this._manager.logger.debug(`[${identifier}] Advanced Information Fetched: ${data.name}`);
            return data;
        }
        catch (error) {
            ServerUtils_1.default.error(this._manager, `Failed To Fetch Advanced Information: ${error}`);
            return null;
        }
    }
    /**
